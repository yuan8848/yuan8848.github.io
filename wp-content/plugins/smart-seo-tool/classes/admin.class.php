...
class Smart_SEO_Tool_Admin {
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		$wb_cnf = array(
			...
			'is_pro'=> get_option('wb_sst_ver',0),
			'action'=> array(
		...
		$in_line_js = 'window.wb_vue_path="'.SMART_SEO_TOOL_BASE_URL.'tpl/"; var wb_cnf='.json_encode($wb_cnf,JSON_UNESCAPED_UNICODE).';';
	...
	public static function get_demo_data()
    {
        ...
        $page = $wpdb->get_row("SELECT * FROM $wpdb->posts WHERE post_type='page' AND post_status='publish' ORDER BY rand() LIMIT 1");
	...
	public static function get_setting($key=''){
		...
		switch ($key){
			...
			case 'url_broken':
				...
				$total = $wpdb->get_var("SELECT COUNT(1) FROM $url_log WHERE url_md5 IS NOT NULL AND ( `code`<>200 OR `code` IS NULL)");
