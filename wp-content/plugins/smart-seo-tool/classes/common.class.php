...
class Smart_SEO_Tool_Common {
	...
	public static function detect_broken_url() {
		...
		$list = $wpdb->get_results( "SELECT * FROM $t WHERE `code`<>'200' AND url_md5 IS NOT NULL AND (check_date IS NULL OR DATE_ADD(check_date ,INTERVAL $day DAY) < NOW() ) LIMIT 30 " );
