...
class WB_BSL_Bing extends WB_BSL_Base
{
    ...
    public static function push_batch_url($urls)
    {
        ...
        if(is_wp_error($http)){
            $ret['desc'] = "批量提交出错，".$http->get_error_message();
        ...
        if(!$data){
            $ret['desc'] = 'API响应解析出错';
            self::error('批量提交接口响应解析出错','Bing推送');
            return $ret;
        }
        if(isset($data['ErrorCode'])){
            $ret['code'] = $data['ErrorCode'];
            $ret['desc'] = 'URL提交出错，'.$data['Message'];
            self::error('批量推送出错，'.$data['Message'],'Bing推送');
    ...
    public static function push_url($url)
    {
        ...
        if(is_wp_error($http)){
            ...
            self::error('接口请求出错，'.$ret['desc'],'Bing推送');
            return $ret;
        }
        ...
        if(isset($data['ErrorCode'])){
            ...
            self::error('推送出错，'.$data['Message'],'Bing推送');
            return $ret;
        }
        $ret['code'] = 0;
        $ret['desc'] = 'success';
    ...
    public static function get_quota(&$ret = array())
    {
        ...
        if(is_wp_error($http)){
            if(is_array($ret)){
                ...
                self::error('获取配额出错，'.$http->get_error_message(),'Bing推送');
            }
            return 0;
        }
        ...
        if(isset($data['ErrorCode'])){
            if(is_array($ret)){
                $ret['code'] = $data['ErrorCode'];
                $ret['desc'] = $data['Message'];
                self::error('获取配额出错，'.$data['Message'],'Bing推送');
            }
            return 0;
        ...
        if(is_array($ret)){
            ...
            self::error('获取配额出错，'.$ret['desc'],'Bing推送');
        }
